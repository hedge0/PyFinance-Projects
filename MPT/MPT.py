...
volList.append(['SPY', benchAnnualVol, benchAnnualReturn, benchRatio, 0])
	...
	if('trailingPE' in data.columns and 'forwardPE' in data.columns):
		...
		if(pastPE > futurePE and pastPE > 15 and pastPE < 100):
			...
			if(annualVol < 75):
				...
				if(ratio > benchRatio):
					volList.append([key, annualVol, annualReturn, ratio, pastPE])
...
for item in sortedList:
	print(str(item[0]) + " has a return of: " + str(round(item[2], 2)) + "% with PE: " + str(item[4]))
	print("When adjusted for volatility: " + str(round(item[1], 2)) + "%, the ratio is : " + str(round(item[3], 2)), end = '\n\n')
	if(str(item[0]) == 'SPY'):
		benchBool = True
	if(len(newTickers) < 10 and benchBool == False):
...
for portfolio in range(numRuns):
	...
	returns = np.dot(weights, yearlyReturns)
	portRet.append(returns)

	var = covMatrix.mul(weights, axis=0).mul(weights, axis=1).sum().sum()
	sd = np.sqrt(var)
	annualSD = sd*np.sqrt(252)
...
for counter, symbol in enumerate(prices.columns.tolist()):
    ...
    newData[symbol+' weight'] = [w[counter] for w in portWeights]
